...
Эталонный дубль: макушка **610**, подбородок **1380**, плечи 1300–1400. Второй дубль (23.09,
[разбор](../../docs/cases/2026-09-23-first-reel.md)): макушка **430**, подбородок **980** →
`y0 = 350`, шов **860** — формула ниже сработала без правок. Голова у каждого дубля своя: замер
не переносится с ролика на ролик.
...
из распознавания чистовика, не на глаз. Если автор номера не называл (только «во-первых»), границы
шагов выбираются по смыслу и согласуются с человеком — поэтому номера просят проговаривать на
съёмке.

**Панели — только в кадрах-сплитах.** Под врезками «В» и «Т» панель убирается: иначе `check`
видит текст под врезкой и сообщает наложение.

**Тексты на экране — из речи.** Заголовки, цифры, названия сервисов и кодовое слово — то, что
сказал автор; ничего от себя.
...
**Только настоящие SVG из этих трёх источников**, логотип не рисуется и не генерируется. Нет
открытого логотипа (так было с GetCourse) — название сервиса текстовой плашкой. **Каждый логотип
смотри на кадре, а не по имени файла:** `instagram-icon` в gilbarbara — одноцветный чёрный, на
тёмной панели вышел чёрным пятном; фирменный градиент взят из thesvg (E33).
...
### 3а. Генерация картинок (gpt-image-2 через подписку Codex)

Когда нужно показать то, чего нет на экране компьютера (еда, здания, товары) или сделать объёмный фон
хука, иконки шагов, телефон с пустым экраном под настоящий интерфейс. Для AI-контента обычно хватает
HyperFrames без генерации. Источники промптов, методика, шаблоны и ловушки —
[`docs/image-prompts.md`](../../docs/image-prompts.md). Модель рисует образ; текст и логотипы — мы.
...
- Потоки рендера — явно: `-w 3` (или по числу ядер), `auto` выбирает один. Ролик 1:08 на
  MacBook Air M2 с `-w 3` — 1 мин 41 с.
- Границы врезок и панелей генератор ищет в словах чистовика **по точному совпадению** слова и
  только в окне между соседними якорями. Поиск по началу слова находит не то слово (E36).
...
3. `hyperframes compare` / `snapshot` — проверочный кадр с зонами на каждом шаге: панель, шов,
   субтитры, лицо.
4. После рендера: длительность = длительность чистовика до кадра. Исходник 60 к/с при рендере
   30 к/с даёт видео на кадр длиннее звука (E35) — сверяй звук до миллисекунды, видео ±1 кадр.

Проверочный кадр с зонами — **на каждом шаге правок**, не только перед финальным рендером. На
первом ролике по репозиторию он поймал хук в две строки, заголовок во всю ширину и чёрный логотип.
...
- **Двойной префикс `xlink:href`** в инлайн-SVG после обработки кодом — ссылки на градиенты не
  разрешаются, заливка пропадает. Префикс один (или `href`), логотип — смотреть на кадре (E34).
- **Одноцветный логотип под фирменным именем** (`instagram-icon` в gilbarbara) — чёрное пятно на
  тёмной панели. Смотреть каждый логотип на кадре (E33).
- **Панель под врезкой** — `check` ругается на наложение. Панели только в сплитах.
- **Якорь врезки по началу слова** нашёл другое слово. Точное совпадение, окно между соседними
  якорями (E36).
